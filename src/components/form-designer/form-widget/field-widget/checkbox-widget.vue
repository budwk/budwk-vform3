  ...
  <form-item-wrapper :designer="designer" :field="field" :rules="rules" :design-state="designState"
    ...
    <div v-if="previewDetail" class="form-render-content" >
       {{ getDetailValue(fieldModel) }}
                     ...
                     <el-checkbox-group v-else ref="fieldEditor" v-model="fieldModel"
                       :disabled="field.options.disabled" :size="widgetSize"
                       @change="handleChangeEvent">
      <template v-if="!!field.options.buttonStyle">
        <el-checkbox-button v-for="(item, index) in field.options.optionItems" :key="index" :value="item.value"
  ...
  export default {
    ...
    methods: {
      getDetailValue(value) {
        ...
        let result = this.field.options.optionItems.filter(item => value.includes(item.value))
        //
